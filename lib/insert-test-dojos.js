...
const dojos = require('../data/dojos.json');
const dojoMembers = require('../data/dojo-members.json');
const dojoleads = require('../data/dojo-leads.json');
    ...
    return Promise.all(
      dojoMembers.map(async (dojoMember) => {
        try {
          ...
          const membership = await getExistingMembership(
            dojoMember.existing,
            dojo,
            user,
          );
