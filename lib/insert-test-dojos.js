...
module.exports = seneca => ({
  createDojos() {
    ...
    return Promise.all(
      dojos.map(async dojo => {
        try {
          const champions = await seneca.actAsync({
            role: 'cd-users',
            cmd: 'list',
          ...
          const lead = await seneca.actAsync({
            role: 'cd-dojos',
            entity: 'lead',
            cmd: 'load',
            query: { id: champ.id },
          ...
          const submittedLead = await seneca.actAsync({
            role: 'cd-dojos',
            ctrl: 'lead',
            cmd: 'submit',
            lead,
  ...
  linkDojoUsers() {
    ...
    function saveUserDojo(dojoMember, userTypes, dojo, user, membership) {
      ...
      return seneca.actAsync(payload);
    }
  },
  ...
  createDojoLeads() {
    return Promise.all(
      dojoleads.map(lead =>
        seneca
          .actAsync({ role: 'cd-users', cmd: 'list', query: { email: lead.email } })
          .then(dojoAdmins => {
    ...
    return Promise.all(
      polls.map(async poll => {
        try {
          ...
          await seneca.actAsync({ role: 'cd-dojos', cmd: 'save_poll_setup', poll });
        } catch (err) {
          throw err;
        }
      })
